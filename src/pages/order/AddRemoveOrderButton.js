  ...
  return (
    <Fragment>
      <IconButton
        ...
        color="secondary"
        onClick={() =>
          handleRemoveItem(item.id, item.price, orderList, dispatch)
        }
      >
        <RemoveIcon />
      </IconButton>
      <Paper elevation={0} variant="outlined" sx={{ width: 40 }}>
        <Typography component="div" variant="h5" align="center">
          {orderList && isOrderInList(item.id, orderList)
            ? orderList[indexOf(item.id, orderList)].quantity
            : 0}
        </Typography>
      </Paper>
      <IconButton
        ...
        color="primary"
        onClick={() =>
          handleAddItem(item.id, item.name, item.price, orderList, dispatch)
        }
      >
        <AddIcon />
      </IconButton>
